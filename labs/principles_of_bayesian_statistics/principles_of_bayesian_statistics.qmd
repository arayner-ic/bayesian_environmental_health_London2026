...
Typically we would run more than one chain to ensure that the model has converged properly, and we would also check the effective sample size of each parameter to ensure that we have enough samples to make inferences about the posterior distributions. We will do this later in the workshop when we look at more complex models.
