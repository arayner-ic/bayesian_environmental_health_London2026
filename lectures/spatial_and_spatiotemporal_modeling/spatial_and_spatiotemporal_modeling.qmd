...
subtitle: "Structured space-time dependence"
author:
...
- Non smoothed estimates of the standardised mortality ratio (SMR):
...
- SMR is estimated independently: does not account for spatial correlation, i.e. makes no use of SMR estimates in other areas of the map, even though these are likely to be similar.
...
## Specify a distribution to account for information
...
-  Mapping the posterior mean RR (or residual RR) does not make full use of the output of the Bayesian analysis that provides, for each area, samples from the whole posterior distribution of the relative risk.
...
##  Example with HPV vaccination in Switzerland
...
## Space-time (Spaghetti plot)
<center>
![](figures/SpagSIRbyRegion.png){width=75%}
</center>

## Space-time (Space by year)
<center>
