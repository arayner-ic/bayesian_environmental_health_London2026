...
::: nonincremental
- Let's translate one model into several languages.
...
Everything that follows is this model, written in different PPLs.
...
The first five produce draws from the posterior. The last two return a fitted approximation instead, and are covered in the approximation section.


## A toy posterior to compare them on {.smaller}
...
Propose $\theta^{*}$ from $q(\cdot \mid \theta^{(t)})$, and accept it with probability
...
## Random-walk Metropolis: the effect of the scale {.smaller}
...
- When the posterior is correlated, only short moves are accepted.
...
- `NIMBLE` assigns it by default to continuous nodes with no conjugate form. `JAGS` uses it where nothing else applies.
...
- The conditional must be available in closed form. Conjugate pairs (normal-normal, Poisson-gamma, beta-binomial) are the reason classical `BUGS` models are written as they are.
<!-- - Where no conjugate form exists, `BUGS` and `JAGS` use adaptive rejection sampling for log-concave conditionals, and slice or Metropolis otherwise. -->


## Gibbs sampling: one coordinate at a time {.smaller}
...
Forty sweeps. Each segment is one conditional draw, so every move is horizontal or vertical, and the chain works along the ridge in small steps.
...
## Gibbs sampling: the cost of correlation {.smaller}
...
The same sampler and the same number of sweeps. Only the correlation between the two parameters has changed.
...
## Adaptive rejection sampling: the envelope {.smaller}
...
- `JAGS` uses it for non-conjugate continuous nodes. `NIMBLE` uses it by default for discrete nodes.


## Slice sampling: one update {.smaller}
...
## Hamiltonian Monte Carlo: six trajectories {.smaller}
...
- You saw the `MCMC` and `INLA` introduction on Day 1. The next session fills `INLA` in a lot more:
...
- The model is the important thing. The software is a choice you make afterwards.
