---
title: "Software and sampling options"
subtitle: "Choosing a probabilistic programming language and related sampling methods"
author:
...
- Hamiltonian is a mathematical tool or function that represents the total energy (kinetic energy plus potential energy) of a physical system
...
- Treat the parameter as a **position** on a landscape given by the negative log posterior, and invent a **momentum** to go with it.
- Think of it a little like turning a bowl upside down and pinging a ball around.
- Each iteration:
  - draw a fresh momentum from a normal
  - roll the particle for L **leapfrog** steps of size ε, following the gradient
  - accept or reject the endpoint with a Metropolis step
- The trajectory endpoint is far away but has almost the same posterior density. That is what gives long, low-correlation jumps and acceptance near 0.8.
- ε is limited by the narrowest direction of the posterior, εL by the widest, so L is roughly the condition number. Badly scaled models need very long trajectories.
- **NUTS** removes the choice of L by extending the trajectory until it doubles back on itself, and tunes ε during warmup.

## The No-U-Turn Sampler (NUTS), briefly {.smaller}

- No-U-Turn Sampler modifies HMC by preventing long doubling back pathways, which can end up where you start.
...
# Some sampling-based software options
...
- Uses Hamiltonian Monte Carlo with the No-U-Turn (NUTS) Sampler.
