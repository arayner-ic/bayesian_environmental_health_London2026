...
subtitle: "`INLA` and other probabilistic programming languages"
author:
...
# Is sampling the only way (introducing `INLA`)?
...
- The Laplace Approximation (`IN'LA'`) means that we approximate the latent field with a Gaussian.
...
- __But__ it's more difficult to *customise*.
...
model <- inla(
  ...
  control.compute = list(config = TRUE) # so that we can simulate 
                                        # draws of posterior
)
```
...
```{.R}
formula_iid_priors <- deaths ~ 1 + f(SIGLA, model = "iid", 
...
- `INLA` formula (custom priors):
```{.R}
formula_partial_pooling <- y ~ 1 + f(SIGLA, model = "iid",
           hyper = list(
             prec = list(
               prior = "pc.prec",
               param = c(1, 0.01)
...
- Run model using inla function:
```{.R}
model_partial_pooling <- inla(
  formula_partial_pooling,
  data = data,
  E = expected,
  family = "poisson",
...
- `INLA` formula:
```{.R}
formula_non_linear <- O ~ 1 + X1 + X2 + X3
```

- Run model using inla function:
```{.R}
model_non_linear <- inla(
  formula = formula_non_linear,
  family = "poisson",
  data = covid_data,
  offset = E,
  control.fixed = list(
    mean = c("(Intercept)" = 0, X1 = 0, X2 = 0, X3 = 0),
    prec = c("(Intercept)" = 1e-6, X1 = 1, X2 = 1, X3 = 1)
  ),
...
## I think I'm <span style="color:#f928d2;">`INLA`</span> (sorry)

- Good, we'll cover more in the practical.

<!-- ## Shout out to `Theo Rashid` -->
