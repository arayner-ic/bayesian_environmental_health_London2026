---
title: "Regression with non-linear terms"
subtitle: "Beyond the linear Bayesian model"
author:
...
ggplot() +
  #geom_hline(yintercept = leg4coef[[1]], linewidth = 1) +, old 
  geom_hline(yintercept = as.numeric(leg4coef[[1]]), linewidth = 1) +
  xlim(-1, 1) +
  ylim(-1, 2) +
  theme_bw() +
  ylab("") +
...
ggplot() +
  ...
  geom_hline(yintercept = as.numeric(leg4coef[[1]]), cex= .6, linewidth = 1) +
  xlim(-1, 1) +
  ylim(-1, 2) +
  geom_line(aes(
...
-   Gaussian kernels (out of the scope)

    -   Correlation is a function of distance

    -   "Continuous domain models"

    -   Exponential, Gaussian, Matern, etc. (correlation functions)

## Random walk of order n

-   Random walk of order 1

    $$x_i = x_{i-1} + \epsilon_i, \quad \epsilon_i \sim N(0, \tau^{-1})$$

-   Random walk of order 2

    $$x_i = 2x_{i-1} - x_{i-2}  + \epsilon_i, \quad \epsilon_i \sim N(0, \tau^{-1})$$

-   This is very similar with the categorical transformation example, but more concise.

    The prior controls smoothness: the strongest it is the smoother the result. (given the data)

## Random walk of order 1 example
...
# Plot using ggplot2 with points and optional line
ggplot(df, aes(x = time, y = value)) +
  geom_point(color = "steelblue", size = 2) +
  geom_line(color = "grey60", linetype = "dashed") +  # optional to show trend
  labs(
    title = "First-Order Random Walk (RW1)",
    x = "Time",
    y = "Value"
  ) +
...
## Random walk of order 2 example
...
sigma <- 1
x <- numeric(n)
x[1:2] <- rnorm(2, mean = 0, sd = sigma)  # Initial values

for (t in 3:n) {
  x[t] <- 2 * x[t - 1] - x[t - 2] + rnorm(1, mean = 0, sd = sigma)
}
...
ggplot(df, aes(x = time, y = value)) +
  ...
  labs(
    title = "Second-Order Random Walk (RW2)",
    x = "Time",
    y = "Value"
  ) +
  theme_minimal(base_size = 14)
...
-   Autoregressive process of order 1:
    ...
    $$x_i = \phi x_{i-1} + \epsilon_i, \quad \epsilon_i \sim N(0, \tau^{-1})$$

-   Autoregressive process of order 2:

    $$x_i = \phi_1 x_{i-1} + \phi_2 x_{i-2}  + \epsilon_i, \quad \epsilon_i \sim N(0, \tau^{-1})$$

-   when $\phi = 0$ we have RW1 or RW2

-   To complete the parametrisation, we need to define priors of $\phi$. Can you think of a prior?
